...
function setType(t) {
  ...
  // Show payment method only for expenses
  document.getElementById('paymentMethodGroup').style.display = t === 'expense' ? 'block' : 'none';
}
...
function saveTransaction() {
  ...
  const payment = txType === 'expense' ? document.getElementById('txPayment').value : 'cash';
  if (!desc || isNaN(amount) || amount <= 0 || !date) {
    alert('Please fill in all fields.');
    return;
  }
  state.transactions.push({ id: Date.now(), desc, amount, date, cat, type: txType, payment });
...
function txHTML(tx, showYear = false) {
  ...
  const paymentLabel = tx.payment === 'upi' ? '📱 UPI' : (tx.payment === 'cash' ? '💵 Cash' : '');
  const metaStr = paymentLabel ? `${tx.cat.replace(/^\S+\s/, '')} · ${dateStr} · ${paymentLabel}` : `${tx.cat.replace(/^\S+\s/, '')} · ${dateStr}`;
  return `
    <div class="tx-item">
      <div class="tx-icon ${tx.type}">${tx.cat.split(' ')[0]}</div>
      <div class="tx-info">
        <div class="tx-desc">${tx.desc}</div>
        <div class="tx-date">${metaStr}</div>
      </div>
...
/* ── TOGGLE MORE TRANSACTIONS ── */
function toggleMoreTransactions() {
  const moreList = document.getElementById('txListMore');
...
/* ── GRAPH ── */
let graphFilter = null; // null = both, 'expense' = only expenses, 'income' = only income
...
function renderGraph() {
  ...
  // Filter data based on graphFilter
  const filteredData = data.map(d => ({
    ...d,
    exp: (graphFilter === 'income' || graphFilter === null) ? d.exp : 0,
    inc: (graphFilter === 'expense' || graphFilter === null) ? d.inc : 0,
  }));
  
  const maxVal = Math.max(1, ...filteredData.map(d => Math.max(d.exp, d.inc)));
  container.innerHTML = filteredData.map((d, idx) => {
    const isCurrent = d.ym === state.currentMonth;
    const expH = graphFilter === 'income' ? 0 : Math.round((d.exp / maxVal) * 90);
    const incH = graphFilter === 'expense' ? 0 : Math.round((d.inc / maxVal) * 90);
    const [, m] = d.ym.split('-');
    return `
      <div class="graph-bar-group${isCurrent ? ' current' : ''}">
        <div class="graph-bar-wrap">
          ${graphFilter !== 'income' ? `<div class="graph-bar expense" style="height:${expH}px" title="${fmt(d.exp)}" onclick="filterGraphData('expense', event)"></div>` : ''}
          ${graphFilter !== 'expense' ? `<div class="graph-bar income"  style="height:${incH}px" title="${fmt(d.inc)}" onclick="filterGraphData('income', event)"></div>` : ''}
        </div>
...
function filterGraphData(type, event) {
  event.stopPropagation();
  if (graphFilter === type) {
    graphFilter = null; // Toggle back to show both
  } else {
    graphFilter = type; // Show only this type
  }
  renderGraph();
}
...
function render() {
  ...
  const upiExp   = expenses.filter(t => t.payment === 'upi').reduce((s, t) => s + t.amount, 0);
  ...
  document.getElementById('cashSpent').textContent   = fmt(cashExp);
  document.getElementById('upiSpent').textContent    = fmt(upiExp);
  ...
  // Render "This Month" section with 4 visible + dropdown for rest
  if (currentView === 'home') {
    const visibleCount = 4;
    const visibleTxs = monthTxs.slice(0, visibleCount);
    const moreTxs = monthTxs.slice(visibleCount);
    
    const txListVis = document.getElementById('txListVisible');
    const txListMore = document.getElementById('txListMore');
    const moreContainer = document.getElementById('txListMoreContainer');
    
    if (monthTxs.length === 0) {
      txListVis.innerHTML = `<div class="empty-state"><span class="big">💸</span>No transactions yet.<br>Tap + to add one.</div>`;
      moreContainer.style.display = 'none';
    } else {
      txListVis.innerHTML = visibleTxs.map(tx => txHTML(tx)).join('');
      if (moreTxs.length > 0) {
        moreContainer.style.display = 'block';
        txListMore.innerHTML = moreTxs.map(tx => txHTML(tx)).join('');
      } else {
        moreContainer.style.display = 'none';
      }
    }
